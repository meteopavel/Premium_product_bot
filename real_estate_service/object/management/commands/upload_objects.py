...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not file_path:
            raise CommandError(
                "Please provide the path to the CSV file using --file"
            )
                ...
                with transaction.atomic():
                    for row in reader:
                        ...
                        contact, created = Contact.objects.get_or_create(
                            ...
                            phone_number=list(
                                row.get("contact_phone_number").split(",")
                            ),
                        )
                        Realty.objects.get_or_create(
                            ...
                            area=(
                                row.get("area")
                                if isinstance(row.get("area"), int)
                                else None
                            ),
                            price=(
                                row.get("price")
        ...
        self.stdout.write(
            self.style.SUCCESS("Successfully uploaded objects from CSV file")
        )
