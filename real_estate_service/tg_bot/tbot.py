...
from object.models import BuldingType, Category, City, Condition, Realty
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        "Start handler, Choose your desteny!", reply_markup=reply_markup
    )
...
async def main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(
                "Очистить фаильтры", callback_data="refresh_all"
            ),
        ],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def area(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("150-1000", callback_data="150-1000")],
...
async def category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    keyboard = []
    async for category in Category.objects.all().values_list(
        "name", flat=True
    ):
        keyboard.append(
...
async def other_menu(
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "Состояние помещения", callback_data="condition"
            )
        ],
        [InlineKeyboardButton("Тип здания", callback_data="building_type")],
        [
            InlineKeyboardButton(
                "Поиск в тексте обьявления", callback_data="text"
            )
        ],
        [
            InlineKeyboardButton("Сбросить", callback_data="refresh_other"),
            InlineKeyboardButton(
...
async def publish_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    await query.edit_message_text(
        text="Выбери период публикации", reply_markup=markup
    )
...
async def condition(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    keyboard = []
    async for condition in Condition.objects.all().values_list(
        "name", flat=True
    ):
        keyboard.append(
    ...
    if "condition" in context.user_data:
        text = ('Выбранное ранее состояние '
                f'помещений:{context.user_data["condition"]}')
    else:
...
async def building_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    keyboard = []
    async for building_type in BuldingType.objects.all().values_list(
        "name", flat=True
    ):
        keyboard.append(
            [InlineKeyboardButton(building_type, callback_data=building_type)]
        )
    reply_markup = InlineKeyboardMarkup(keyboard)
    context.user_data["choose"] = "building_type"
    if "building_type" in context.user_data:
        text = ('Выбранное ранее состояние помещений:'
                f'{context.user_data["building_type"]}')
    else:
...
def main(token) -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            MAIN_MENU: [
                CallbackQueryHandler(main_menu, pattern=filter_main_menu),
            ],
            SAVE_OTHER_CHOOSE: [
                CallbackQueryHandler(save_other_choose),
            ],
            CHOOSE_OTHER: [
