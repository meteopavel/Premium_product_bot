...
class TGBot:
    def __init__(self):
        ...
        self.ptb_app.add_handler(ConversationHandler(
            entry_points=[CallbackQueryHandler(button, pattern=r"^review_")],
            states={
                ASK_FOR_REVIEW: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_review)],
            },
            fallbacks=[CallbackQueryHandler(cancel_handler, pattern=r"^cancel$")]
        ))
        ...
        self.ptb_app.add_handler(CallbackQueryHandler(back_to_list_handler, pattern=r"^back_to_list$"))
        self.ptb_app.add_handler(CallbackQueryHandler(cancel_handler, pattern=r"^cancel$"))
        self.ptb_app.add_handler(CallbackQueryHandler(realty_callback_handler, pattern=r"^realty_"))
        self.ptb_app.add_handler(CallbackQueryHandler(page_navigation_handler, pattern=r"^page_"))
        self.ptb_app.add_handler(CallbackQueryHandler(show_realty, pattern=r"^realty_"))
        self.ptb_app.add_handler(CallbackQueryHandler(button, pattern=r"^review_"))
        self.ptb_app.add_handler(CallbackQueryHandler(button, pattern=r"^view_reviews_"))
