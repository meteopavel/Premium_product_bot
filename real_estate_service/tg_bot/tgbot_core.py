...
class TGBot:
    def __init__(self):
        ...
        self.ptb_app.add_handler(CallbackQueryHandler(
            show_realty.show_realty, pattern=r"^realty_"))
        self.ptb_app.add_handler(CallbackQueryHandler(
            review_handler.button, pattern=r"^review_"))
        self.ptb_app.add_handler(CallbackQueryHandler(
            review_handler.button, pattern=r"^view_reviews_"))
        self.ptb_app.add_handler(MessageHandler(
            filters.TEXT & ~filters.COMMAND, review_handler.receive_review))
        self.ptb_app.add_handler(CommandHandler(
            "my_favorites", favorites_handler.get_favorites))
        self.ptb_app.add_handler(CallbackQueryHandler(
            favorites_handler.add_to_favorites, pattern=r"^add_to_favorite_"))
        self.ptb_app.add_handler(CallbackQueryHandler(
            favorites_handler.delete_favorite, pattern=r"^delete_favorite_"))
        self.ptb_app.add_handler(MessageHandler(
            filters.TEXT & ~filters.COMMAND, echo_handler.echo))
